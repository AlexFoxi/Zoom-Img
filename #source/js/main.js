...
const ImageZoom = (props) => {
  ...
  let { defZoomLevel = 0.5 } = props;
  if (defZoomLevel < minZoom) defZoomLevel = minZoom;
...
const fullScreenSlider = () => {
  ...
  [...containers].forEach((container) => {
    ...
    function closeSlider() {
      ...
      document
        .querySelectorAll('.fullscreen-container .slider-slide')
    ...
    function sliderInteractions(target) {
      ...
      const showSlide = (index) =>
        (sliderWrapper.style.transform = `translateX(-${
          index * (compWidth + compMarginRight)
        }px)`);
      ...
      document
        .querySelector('.fullscreen-container.visible')
